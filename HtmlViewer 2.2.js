console.log("正在加载 HtmlViewer 2.2.0。\n作者：Ruize Sun\n感谢使用！");
...
class HtmlViewer {
	...
	showHtml(args) {
		...
		div.getElementsByClassName("HVTitle")[0].innerHTML = args.title.replaceAll("<", "").replaceAll(">", "");
	...
	setHtmlViewerCoordinate(args) {
		// 设置窗口在屏幕上的坐标
		let hv = document.getElementById("HtmlViewerWindow");
		hv.setAttribute("style", "top: " + args.top + "px;left: " + args.left + "px;");
	}

	getWindowTitle() {
		// 获取窗口标题
		return document.querySelector(".HtmlViewerWindowTitle > span").innerHTML;
	}

	getWindowContent() {
		// 获取窗口内容
		return document.querySelector(".HtmlViewerBox").innerHTML;
	}

	changeWindow(args) {
		// 更改窗口信息
		div.getElementsByClassName("HtmlViewerBox")[0].innerHTML = markdownToHtmlS(args.html);
		div.getElementsByClassName("HVTitle")[0].innerHTML = args.title.replaceAll("<", "").replaceAll(">", "");
	}
}
